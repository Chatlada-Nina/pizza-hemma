...
class StripeWH_Handler:
    """
    Handle Stripe webhook events for the payment system.

    This class is designed to handle Stripe webhook events for various payment
    status updates, including successful payments, failed payments, and other
    events. The handler processes the event data, updates the order details,
    and manages user profile updates where necessary.

    Methods:
    - __init__(request): Initializes the handler with the given HTTP request.
    - _send_confirmation_email(order): Sends a confirmation email to the
    customer for a processed order.
    - handle_event(event): Handles a generic/unexpected Stripe webhook event.
    - handle_payment_intent_succeeded(event): Handles a successful payment
    intent webhook event from Stripe.
    - handle_payment_intent_payment_failed(event): Handles a failed payment
    intent webhook event from Stripe.
    """
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        try:
            ...
            # Recalculate grand total if deli-cost isn't provided in metadata.
            ...
            # Recalculate deli-cost if it's not provided in the metadata.
            if delivery_cost == 0:
            ...
            if username != 'AnonymousUser':
                ...
                if profile and save_info:
                    ...
                    profile.default_town_or_city = \
                        shipping_address.get('city', 'Gothenburg')
                    profile.default_country = \
                        shipping_address.get('country', 'SE')
            ...
            filter_kwargs = {key: value for key, value in filter_kwargs.items()
            ...
            try:
                ...
                for item_id, item_data in json.loads(cart).items():
                    ...
                    OrderLineMenu.objects.create(
                        order=order, menu=item, quantity=item_data
                    )
    ...
    def handle_payment_intent_payment_failed(self, event):
        ...
        try:
            ...
            return HttpResponse(
                content=f'Error handling payment failed event: {e}',
                status=500
            )
